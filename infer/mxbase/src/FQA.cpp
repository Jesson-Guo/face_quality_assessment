...
int PreProcess(const std::string& imgPath, std::vector<MxBase::TensorBase>* inputs) {
    cv::Mat imageMat;
    int height, width;
...
bool Readgt(const std::string& txtPath, float& eulgt[3], int& kp_list[5][2], int width, int height) {
...
int InferResult(auto heatmap, int& kp_coord_ori[5][2]) {
    for (int i = 0; i < 5; i++) {
...
int PrintResult(
    ...
    for (uint32_t i = 1; i < kp_ave_error.size(); i++) {
        std::cout << ", " << kp_ave_error[i];
    }
    std::cout << "]\n";
    std::cout << "========== 3 eulers average err: [" << euler_ave_error[0];
    for (uint32_t i = 1; i < euler_ave_error.size(); i++) {
        std::cout << ", " << euler_ave_error[i];
    }
    std::cout << "]\n";
    float ipn(0), mae(0);
...
APP_ERROR FQA::Process(const std::string& testPath) {
    ...
    while ((entry = readdir(directory_pointer)) != NULL) {
        ...
        bool euler_kps_do = Readgt(txtPath, euler_kps_do, eulgt, kp_list, width, height);
        if (!euler_kps_do) {
            continue;
        }
